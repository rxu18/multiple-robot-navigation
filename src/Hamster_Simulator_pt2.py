...
'''
'''
This hamster simulator is complex. We only modified the intersect and in_collision.
for in_collision, the idea is that:
    for each side of the rectangle:
        for each side of the hamster:
            check if they intersect or not.
To check each side of the hamster, we need to do some trigonometry. Now the weird thing
about the way angles are stored is that up/North is 0 degrees, and clockwise is positive,
instead of +x being 0 degrees, and counterclockwise being positive. This is fixed by 
reflecting the coordinates over x=y.
Now, to check whether two line segments intersect, we use the intersect method. The
first part is mainly just obtaining the oint of intersection (x,y), and the two
if statements are there to make sure that (x,y) is actually on the line.
...
def intersect(line1, line2):
    ...
    if div == 0:
        return False
    d = (line1[0] * line1[3] - line1[1] * line1[2],
         line2[0] * line2[3] - line2[1] * line2[2])
    x = det(d, xdiff) / div
    y = det(d, ydiff) / div
    #print ("x = %d, y = %d" % (x,y))
    if ((x - line1[0] > -0.01 and x - line1[2] < 0.01) or (x - line1[2] > -0.01 and x - line1[0] < 0.01)) and ((x - line2[0] > -0.01 and x - line2[2] < 0.01) or (x - line2[2] > -0.01 and x - line2[0] < 0.01)):
        if ((y - line1[1] > -0.01 and y - line1[3] < 0.01) or (y - line1[3] > -0.01 and y - line1[1] < 0.01)) and ((y - line2[1] > -0.01 and y - line2[3] < 0.01) or (y - line2[3] > -0.01 and y - line2[1] < 0.01)):
            return True
    return False
...
class virtual_world:
    ...
    def draw_map(self):
        ...
        for rect in self.map:
            ...
            self.canvas.create_rectangle(
                [x1, y1, x2, y2], outline="grey", fill="grey")
    ...
    def draw_robot(self):
        ...
        if (self.trace):
            ...
            self.canvas.create_polygon(
                [x1, y1, x2, y2, x3, y3], outline="blue")
            ...
            if (a_r >= 0) and (a_r < 3.1415/2):
                ...
                if (y_e < y1):
                    ...
                    if (x_i > x1 and x_i < x2):
                        # 1 indicating intersecting a bottom edge of obs
                        p_new = [x_i, y_i, 1]
                if (x_e < x1):
                    x_i = x1
    ...
    def get_vrobot_prox(self, side):
        ...
        if (side == "left"):
            ...
            a_e = vrobot.a + 3.1415/4.5  # emitter location
        x_e = (vrobot.l-2) * math.sin(a_e) + \
            vrobot.x  # emiter pos of left sensor
        y_e = (vrobot.l-2) * math.cos(a_e) + \
            vrobot.y  # emiter pos of right sensor
        ...
        if intersection:
            ...
            y_i = intersection[1]
            if (side == "left"):
                vrobot.dist_l = math.sqrt(
                    (y_i-y_e)*(y_i-y_e) + (x_i-x_e)*(x_i-x_e))
                if vrobot.dist_l > 120:
                    vrobot.dist_l = False
                return vrobot.dist_l
            else:
                vrobot.dist_r = math.sqrt(
                    (y_i-y_e)*(y_i-y_e) + (x_i-x_e)*(x_i-x_e))
    ...
    def draw_prox(self, side):
        ...
        if (prox_dis):
            x_e = (vrobot.l-4) * math.sin(a_e) + \
                vrobot.x  # emiter pos of left sensor
            y_e = (vrobot.l-4) * math.cos(a_e) + \
                vrobot.y  # emiter pos of right sensor
            x_p = prox_dis * math.sin(vrobot.a) + x_e
            y_p = prox_dis * math.cos(vrobot.a) + y_e
            if (self.prox_dots):
                self.canvas.create_oval(canvas_width+x_p-1, canvas_height-y_p-1,
                                        canvas_width+x_p+1, canvas_height-y_p+1, outline='red')
            point_list = (canvas_width+x_e, canvas_height-y_e,
                          canvas_width+x_p, canvas_height-y_p)
    ...
    def draw_floor(self, side):
        ...
        points = (canvas_width+x_f-2, canvas_height-y_f-2,
                  canvas_width+x_f+2, canvas_height-y_f+2)
        self.canvas.coords(floor_id, points)
        if (border):
            self.canvas.itemconfig(floor_id, outline="black", fill="black")
            if (self.floor_dots):
                self.canvas.create_oval(canvas_width+x_f-2, canvas_height -
                                        y_f-2, canvas_width+x_f+2, canvas_height-y_f+2, fill='black')
        else:
    ...
    def in_collision(self):
        s = self.vrobot.a + (3.1415/4)
        vtx = []
        # Generates the vertices of the robot, by using trigonometry!
        for i in range(4):
            vtx.append((self.vrobot.x + self.vrobot.l * math.sin(s),
                        self.vrobot.y + self.vrobot.l * math.cos(s)))
            s += 3.1415 / 2
        roboLines = []
        # Generates the outline of the robot.
        for i in range(4):
            roboLines.append(
                (vtx[i][0], vtx[i][1], vtx[(i + 1) % 4][0], vtx[(i+1) % 4][1]))
        # for i in range(4):
        #     print str(roboLines[i]);
        #     print intersect(roboLines[i], [40, -50, 40, 50])
        # print str(roboLines)
        # print str(roboLines[0])
        # print intersect(roboLines[0], [40, -50, 40, 50]);
        for rect in self.map:
            rectLines = []
            rectLines.append((rect[0], rect[1], rect[2], rect[1]))
            rectLines.append((rect[2], rect[1], rect[2], rect[3]))
            rectLines.append((rect[2], rect[3], rect[0], rect[3]))
            rectLines.append((rect[0], rect[3], rect[0], rect[1]))
            for rectLine in rectLines:
                for roboLine in roboLines:
                    if (intersect(roboLine, rectLine)):
                        return True
        return False
